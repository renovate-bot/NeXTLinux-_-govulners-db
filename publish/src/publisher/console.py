...
import publisher.utils.govulners as govulners
...
GOVULNERS_TEST_SCHEMA = os.environ.get("GOVULNERS_TEST_SCHEMA", None)
GOVULNERS_TEST_RELEASE = os.environ.get("GOVULNERS_TEST_BRANCH", None)


@click.group(help="Tooling to support generating the publishing supported versions of govulners DB.")
def cli():
...
def generate(schema_version: int):
    ...
    # TODO: use a released version of govulners-db
    logging.info(f"using govulners-db from main branch")

    db_builder = builder.GovulnersDbBuilder()
    govulners_obj = govulners.Govulners(schema_version=schema_version, release=GOVULNERS_TEST_RELEASE)

    # build set of test cases derived from the govulners version and its configured schema versions
    runner.add_case(
        case=test.Case(
            govulners=govulners_obj, schema_version=schema_version, builder=db_builder
        )
    )
...
def upload_listing(s3_bucket: str, s3_path: str, dry_run: bool):
    ...
    # TODO: test out the new listing file with all supported versions of govulners
    logging.info("acceptance testing the new listing")
    override_schema_release = None
    if GOVULNERS_TEST_SCHEMA and GOVULNERS_TEST_RELEASE:
        override_schema_release = (GOVULNERS_TEST_SCHEMA, GOVULNERS_TEST_RELEASE)
...
def listing_entries_dbs_in_s3(
    ...
    for basename in basenames:
        ...
        with tempfile.TemporaryDirectory(prefix="govulners-downloaded-db") as tempdir:
