...
from publisher.utils.constants import CACHE_DIR, GOVULNERS_DB_CONFIG, MAX_ALLOWABLE_DB_MB_SIZE


class GovulnersDbBuilder:
    def __init__(self):
    ...
    @classmethod
    def run(cls, *args, cache_dir=CACHE_DIR, config=GOVULNERS_DB_CONFIG, caller=subprocess.check_call):
        cmd = " ".join(["go run ./cmd/govulners-db/main.go", *args])
        print(f"running {cmd!r}")

        env = dict(
            **os.environ.copy(),
            **{
                "GOVULNERS_DB_PROVIDER_ROOT": cache_dir,
                "GOVULNERS_DB_CONFIG": config,
            },
        )
