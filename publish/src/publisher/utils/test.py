...
@dataclass
class Case:
    govulners: "govulners.Govulners"
    builder: "builder.GovulnersDbBuilder"
    schema_version: int
...
class Runner:
    ...
    def add_case(self, case: Case):
        ...
        if existing_case.govulners.release.version < case.govulners.release.version:
            self.cases[case.schema_version] = case
    ...
    def run_test_case(self, case: Case):
        logging.info(
            f"running acceptance test for schema={case.schema_version} govulners={case.govulners.release}"
        )
        ...
        try:
            ...
            case.govulners.import_db(db_path=db_path)

            output = case.govulners.run(user_input=self.user_input)

            # run the test and ensure the output is "good enough"
            current_run = govulners.Report(report_contents=output)
            golden_run = govulners.Report(report_contents=golden_report_content)

            # either raises an error or returns nothing based on the comparison
            golden_run.compare(other=current_run)

        except Exception as exc:
            logging.error(f"failed test case={case.govulners.release}: {str(exc)}")
            raise
