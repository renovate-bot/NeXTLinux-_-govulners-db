...
from yardstick.tool import govulners
...
# we know that there could be slight differences in govulners output between versions
...
class Govulners:
    ...
    def __init__(self, schema_version: int, update_url: str = "", release: Optional[str] = None):
        ...
        if release:
            logging.warning(f"overriding govulners release for schema={schema_version!r} with release={release!r}")
            self.release = release
        else:
            self.release = self.release_version_for_schema_version(schema_version)
        logging.debug(f"using govulners release={self.release!r} for schema={schema_version!r}")

        env = {}
        if update_url:
            env["GOVULNERS_DB_UPDATE_URL"] = update_url
        self.tool = govulners.Govulners.install(version=self.release, path=os.path.join(ASSET_DIR, self.release), env=env)

    @staticmethod
    def release_version_for_schema_version(schema_version: int):
        path = os.path.join(repo_root(), "govulners-schema-version-mapping.json")
...
class Report:
    ...
    def _enumerate(self, section):
        try:
            ...
            report_path = os.path.join(ERROR_DIR, "govulners-error.json")
    ...
    def parse(self) -> Tuple[Set["Package"], Set["Vulnerability"]]:
        ...
        for entry in self._enumerate(section="matches"):
            # not all versions of govulners included epoch in the version, so for comparison it is vital that
...
def check_db_cache_dir(schema_version, db_runtime_dir):
    ...
    # ensure the db cache is not empty for the current schema
    if schema_version == "1":
        # older govulners versions do not support schema-based cache directories
    ...
    if os.path.exists(db_metadata_file):
        # the metadata.json file exists and govulners will be able to work with it
        return
