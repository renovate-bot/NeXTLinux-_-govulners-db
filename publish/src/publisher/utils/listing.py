...
def acceptance_test(test_listing: Listing, image: str, override_schema_release: Optional[Tuple[str, str]] = None):
    # write the listing to a temp dir that is served up locally on an HTTP server. This is used by govulners to locally
    # download the listing file and check that it works against S3 (since the listing entries have DB urls that
    # reside in S3).
    with tempfile.TemporaryDirectory(prefix="govulners-db-acceptance") as tempdir:
        ...
        # ensure govulners can perform a db update for all supported schema versions. Note: we are only testing the
        # listing entry for the DB is usable (the download succeeds and govulners and the update process, which does
        ...
        with http_server(directory=tempdir) as listing_url:
            if override_schema_release:
                ...
                logging.info(f"testing govulners schema-version={override_schema!r}")
                tool_obj = govulners.Govulners(
                    schema_version=override_schema,
                    update_url=listing_url,
                    release=override_release
                )
            else:
                for schema_version in govulners.Govulners.supported_schema_versions():
                    logging.info(f"testing govulners schema-version={schema_version!r}")
                    tool_obj = govulners.Govulners(
            ...
            packages, vulnerabilities = govulners.Report(report_contents=output).parse()
