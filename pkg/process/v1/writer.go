...
import (
	...
	govulnersDBStore "github.com/nextlinux/govulners/govulners/db/v1/store"
)
...
func normalizeSeverity(metadata *govulnersDB.VulnerabilityMetadata, reader govulnersDB.VulnerabilityMetadataStoreReader) {
	...
	if strings.Contains(metadata.RecordSource, govulnersDB.NVDNamespace) {
		return
	}
	m, err := reader.GetVulnerabilityMetadata(metadata.ID, govulnersDB.NVDNamespace)
	if err != nil {
