...
SOURCE_REPO_URL = https://github.com/nextlinux/govulners-db
TEMP_DIR = ./.tmp
RESULTS_DIR = $(TEMP_DIR)/results

DB_ARCHIVE = ./govulners-db-cache.tar.gz
GOVULNERS_DB = go run ./cmd/$(BIN)/main.go -c publish/.govulners-db.yaml
GOVULNERS_DB_DATA_IMAGE_NAME = ghcr.io/nextlinux/$(BIN)/data
...
unit-go: ## Run GO unit tests (with coverage)
	...
	go test -coverprofile $(TEMP_DIR)/unit-coverage-details.txt $(shell go list ./... | grep -v nextlinux/govulners-db/test)
...
acceptance: ## Run acceptance tests (for local use, not CI)
	...
	cd test/acceptance && poetry run python ./govulners-ingest.py test-all

.PHONY: cli
cli: $(SNAPSHOT_DIR)  ## Run CLI tests
	chmod 755 "$(SNAPSHOT_BIN)"
	$(SNAPSHOT_BIN) version
	GOVULNERS_DB_BINARY_LOCATION='$(SNAPSHOT_BIN)' \
...
update-test-fixtures:
	docker run \
		...
		--rm \
		-it \
		nextlinux/govulners:latest \
			-q \
		 	-o json \
		 		centos:8.2.2004 > publish/test-fixtures/centos-8.2.2004.json
	dos2unix publish/test-fixtures/centos-8.2.2004.json
	cd test/acceptance && poetry install && poetry run python govulners-ingest.py capture-test-fixtures
...
show-providers:
	...
	@$(GOVULNERS_DB) list-providers -q -o json
...
download-provider-cache:
	...
	@bash -c "oras pull $(GOVULNERS_DB_DATA_IMAGE_NAME)/$(provider):$(date) && $(GOVULNERS_DB) cache restore --path $(DB_ARCHIVE) || (echo 'no data cache found for today' && exit 1)"

.PHONY: refresh-provider-cache
refresh-provider-cache:
	$(call title,Refreshing "$(provider)" provider data cache)
	$(GOVULNERS_DB) pull -v -p $(provider)
...
upload-provider-cache: ci-check
	...
	@rm -f $(DB_ARCHIVE)
	$(GOVULNERS_DB) cache status -p $(provider)
	$(GOVULNERS_DB) cache backup -v --path $(DB_ARCHIVE) -p $(provider)
	oras push -v $(GOVULNERS_DB_DATA_IMAGE_NAME)/$(provider):$(date) $(DB_ARCHIVE) --annotation org.opencontainers.image.source=$(SOURCE_REPO_URL)
	$(TEMP_DIR)/crane tag $(GOVULNERS_DB_DATA_IMAGE_NAME)/$(provider):$(date) latest
...
upload-all-provider-cache: ci-check
	...
	@rm -f $(DB_ARCHIVE)
	$(GOVULNERS_DB) cache status
	$(GOVULNERS_DB) cache backup -v --path $(DB_ARCHIVE)
	oras push -v $(GOVULNERS_DB_DATA_IMAGE_NAME):$(date) $(DB_ARCHIVE) --annotation org.opencontainers.image.source=$(SOURCE_REPO_URL)
	$(TEMP_DIR)/crane tag $(GOVULNERS_DB_DATA_IMAGE_NAME):$(date) latest
...
download-all-provider-cache:
	...
	@rm -f $(DB_ARCHIVE)
	@bash -c "oras pull $(GOVULNERS_DB_DATA_IMAGE_NAME):$(date) && $(GOVULNERS_DB) cache restore --path $(DB_ARCHIVE) || (echo 'no data cache found for today' && exit 1)"
